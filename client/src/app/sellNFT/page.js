...
export default function SellNFT() {
  ...
  //here we are using fileURL to upload data to pinata
  const [fileURL, setFileURL] = useState();
  //to show error while getting the file we will use message
  const [message, updateMessage] = useState("");
  //btn = button keeps track of when to activatre and deactivate the button
  const [btn, setBtn] = useState(false);
  //btnContent stores the content of the button
  const [btnContent, setBtnContent] = useState("List NFT");
  //we are using router here to wsend the user back to homepage after NFT has been uploaded
  const router = useRouter();
  //This is checking if the user is connected or not
